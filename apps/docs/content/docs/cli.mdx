...
| Command | Purpose |
|---|---|
| `multica mcp-stdio` | Run the Multica MCP server over stdio — expose the active workspace as tools for Claude Code, Cursor, Windsurf, and other MCP-aware clients. Auth reads from `~/.multica/config.json` (the same config as every other CLI command). See [Ship MCP](/ship-mcp) for setup instructions and the full tool reference. |
