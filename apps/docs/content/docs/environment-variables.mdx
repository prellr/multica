...
| `GITHUB_APP_ID` | empty | Numeric App ID from the GitHub App's General page. When set together with `GITHUB_APP_PRIVATE_KEY`, Ship Hub authenticates as the App (own per-installation rate quota) for every backend GitHub call — PR sync, CI status, workflow polling, release linkage |
| `GITHUB_APP_PRIVATE_KEY` | empty | PEM-encoded RSA private key downloaded from the App's "Private keys" section. Paste the full PEM (including `-----BEGIN/END-----` lines) as a multi-line quoted value |

**Behavior when either of the first two is unset:**
...
**Behavior when `GITHUB_APP_ID` / `GITHUB_APP_PRIVATE_KEY` is unset:** Ship Hub falls back to the per-workspace personal token configured in Settings → GitHub. That token shares the operator's 5000-req/hr REST quota with every agent's `gh` CLI traffic — fine for low volume, but the App path is recommended for any deployment with frequent deploys or autopilot activity.
