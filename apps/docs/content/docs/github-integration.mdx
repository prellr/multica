...
| **Permissions → Repository → Pull requests** | **Read & write** (Ship Hub merges PRs from the merge-train). Read-only is enough for the issue-link integration on its own. |
| **Permissions → Repository → Metadata** | Read-only (mandatory). |
| **Permissions → Repository → Contents** | **Read-only** (Ship Hub reads `.github/workflows/*.yml` to introspect the deploy pipeline). |
| **Permissions → Repository → Actions** | **Read-only** (Ship Hub polls workflow runs for staging / production deploys). |
| **Permissions → Repository → Deployments** | **Read & write** (Ship Hub creates GitHub `deployment` objects for the timeline). |
| **Permissions → Repository → Issues** | **Read-only** (link-back on tracking issues). |
| **Subscribe to events** | Tick **Pull request**, **Push**, **Check run**, **Workflow run**, **Deployment status**. |
| **Where can this GitHub App be installed?** | Your choice. `Only on this account` is fine for single-org setups. |

After **Create GitHub App**, note four things from the App's detail page:
...
- The numeric **App ID** at the top of the General tab.
- Scroll to **Private keys** → **Generate a private key**. The browser downloads a `.pem` file — keep it; you cannot re-download it.
...
# Ship Hub: authenticate as the App (own per-installation rate-limit
# quota, independent of any personal PATs). Both required together; if
# either is empty, Ship Hub falls back to the per-workspace personal
# token stored in Settings → GitHub → "Personal access token".
GITHUB_APP_ID=<numeric App ID from the App's General page>
GITHUB_APP_PRIVATE_KEY="-----BEGIN RSA PRIVATE KEY-----
<paste the full contents of the .pem you downloaded, including newlines>
-----END RSA PRIVATE KEY-----"
```
...
`GITHUB_APP_ID` and `GITHUB_APP_PRIVATE_KEY` are recommended but optional. When both are set, Ship Hub mints short-lived installation access tokens for every backend GitHub call (PR sync, CI status, workflow run polling, release linkage). Each installation gets its own 5000-req/hr quota, so Ship Hub no longer competes with `gh` CLI traffic from your agents. When either is empty the legacy per-workspace personal-token path is used.

<Callout type="info">
**Pasting the private key.** Docker Compose and most env loaders accept multi-line values when quoted. If your secret manager strips newlines, replace literal `\n` in the PEM with real newlines on the way in, or mount the `.pem` as a file and `cat` it into the variable. Common signs of a malformed key: backend logs `github app auth: parse private key: ...` at boot, and Ship Hub falls back to the PAT path silently.
</Callout>
