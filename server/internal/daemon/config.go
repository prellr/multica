...
const (
	...
	// (DefaultAgentIdleWatchdog / DefaultAgentToolWatchdog), so a session that
	// keeps emitting events is never killed merely for running long (upstream
	// MUL-3064, ported in this fork). Operators who want a hard ceiling for
	// cost/resource control can still set MULTICA_AGENT_TIMEOUT to a positive
	// duration; the server-side 2.5h sweeper remains the coarse backstop.
	...
	// forever, so this watchdog is its sole liveness net. The previous 5 min
	// default killed legitimate long assistant outputs (e.g. RFC-length
	// writeups) where the model streams a single message for many minutes
	// without any daemon-visible activity — see MUL-2300. 30 min keeps the
	// safety net for truly stuck runs (dockerd hang) while leaving headroom
	// for long writes. Set MULTICA_AGENT_IDLE_WATCHDOG=0 to disable.
	DefaultAgentIdleWatchdog = 30 * time.Minute
	// DefaultAgentToolWatchdog bounds how long a single tool call may stay in
	// flight (tool_use emitted, no tool_result and no other message) before
	// the idle watchdog force-stops the run. The idle watchdog ignores its
	// normal window while a tool is in flight, because a real build/install/
	// test legitimately runs silently for many minutes — but with no
	// wall-clock cap (DefaultAgentTimeout = 0) a backend that emits tool_use
	// and never the matching tool_result would otherwise run forever. This
	// is the backstop for that stuck-tool case (upstream MUL-3064). Set
	// MULTICA_AGENT_TOOL_WATCHDOG=0 to disable, in which case an in-flight
	// tool never force-stops the run (the historical behavior of the fork's
	// pre-port watchdog).
