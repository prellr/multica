...
func (d *Daemon) executeAndDrain(ctx context.Context, backend agent.Backend, prompt string, opts agent.ExecOptions, taskLog *slog.Logger, taskID string) (agent.Result, int32, error) {
	...
	// Bound the drain loop only when there is a wall-clock cap. With a
	// positive opts.Timeout, give the drain a slightly longer deadline than
	// the backend so it can still collect the backend's own timeout Result
	// if the scanner is stuck on a hung stdout pipe (the extra 30 s covers
	// cleanup after the backend's own deadline fires). With no cap
	// (opts.Timeout <= 0) the inactivity watchdog is the only liveness net,
	// so the drain must NOT impose its own deadline either — otherwise an
	// actively streaming long run would be cut off here regardless of
	// progress (upstream MUL-3064).
	...
	// window — so the failure message reports the real duration that fired.
...
// threshold (so the failure message reports the real duration), sets fired,
// and calls cancel, which propagates to the agent subprocess (via the ctx
// passed to backend.Execute) and to drainCtx. The silence budget depends on
...
//     flight (the fork's pre-port watchdog). Without this in-flight budget a
//     backend that emits tool_use and never the matching tool_result would run
//     forever now that there is no wall-clock cap (upstream MUL-3064).
//
...
func (d *Daemon) runIdleWatchdog(agentCtx context.Context, window, toolWindow time.Duration, lastActivityAt *atomic.Int64, inFlightTools *atomic.Int32, fired *atomic.Bool, firedThreshold *atomic.Int64, cancel context.CancelFunc, messages <-chan agent.Message, taskLog *slog.Logger, taskID string) {
	...
	for {
		...
		case <-ticker.C:
			...
			// toolWindow <= 0 disables the in-flight bound entirely (the
			// pre-port behavior, preserved for operators who explicitly
			// set MULTICA_AGENT_TOOL_WATCHDOG=0).
			threshold := window
