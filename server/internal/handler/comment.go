...
//
// Reconciled with upstream during the sync merge: uses GetAgentInWorkspace
// (workspace-scoped lookup, prevents a mention from resolving to an agent in
// a different workspace) + canAccessPrivateAgent (the upstream-side helper
// that handles the member→private allowed_principals check and the
// agent→agent always-pass case in one place).
func (h *Handler) enqueueAgentMention(ctx context.Context, issue db.Issue, comment db.Comment, agentID, authorType, authorID, wsID string) {
	...
	// Load the agent scoped to the current issue's workspace. Using the bare
	// GetAgent would let a mention resolve to an agent in a different
	// workspace, and the visibility check below would then be applied
	// against the wrong workspace's roles.
