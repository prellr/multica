...
func (h *Handler) ListReferencingIssues(w http.ResponseWriter, r *http.Request) {
	id := chi.URLParam(r, "id")
	issue, ok := h.loadIssueForUser(w, r, id)
	if !ok {
		return
	}
	refs, err := h.Queries.ListReferencingIssues(r.Context(), db.ListReferencingIssuesParams{
		WorkspaceID: issue.WorkspaceID,
		ID:          issue.ID,
	})
	if err != nil {
		writeError(w, http.StatusInternalServerError, "failed to list referencing issues")
		return
	}
	prefix := h.getIssuePrefix(r.Context(), issue.WorkspaceID)
	resp := make([]IssueResponse, len(refs))
	for i, ref := range refs {
		resp[i] = issueToResponse(ref, prefix)
	}
	writeJSON(w, http.StatusOK, map[string]any{
		"issues": resp,
	})
